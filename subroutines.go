...
func (pp PublicParameter) rpulpVerify(cmts []*Commitment, n int,
	...
	chmp, err := pp.expandChallenge(rpulppi.chseed) // TODO_DONE:hanle the err
	if err != nil {
		return false, nil
	}
	ch := pp.NTT(chmp)
...
elrsSignRestart:
	...
	for j := (sidx + 1) % ringSize; ; {
		...
		chtmm, err := pp.expandChallenge(seedj) //TODO_DONE:handle the err
		if err != nil {
			return nil, err
		}
		chj = pp.NTT(chtmm)
...
func (pp *PublicParameter) elrsVerify(t_as []*PolyNTTVec, t_cs []*PolyNTTVec, msg []byte, elrssig *elrsSignature) (valid bool,err error) {
	//	check the well-formness of inputs
	if t_as == nil || t_cs == nil || msg == nil {
		return false,errors.New("error in") // TODO: define the const error
	}
	...
	for j := 0; j < ringSize; j++ {
		...
		if err != nil {
			...
			return false,errors.New("error in")// TODO: define the const err
		}
